...
<html>
    ...
    <%
        NewsPageBean newsPageBean=(NewsPageBean)request.getSession().getAttribute("newspagebean");
    %>
	<body bgcolor="#6CA6CD">
		<center>
					...
					<td height="254" colspan="2">
						<table width="100%" border="0">
							<tr align="center">
								...
								<td width="5%">
									<input type="button" value="首页" onclick="window.location.replace('../ShowNewsTable?pageNow=1')">
								</td>
								<td width="5%">
									<input type="button" value="上一页" onclick="window.location.replace('../ShowNewsTable?pageNow=<%=newsPageBean.getCrruntPage()-1%>')">
								</td>
								<td width="5%">
									<input type="button" value="下一页" onclick="window.location.replace('../ShowNewsTable?pageNow=<%=newsPageBean.getCrruntPage()+1%>')">
								</td>
								<td width="5%">
									<input type="button" value="尾页" onclick="window.location.replace('../ShowNewsTable?pageNow=<%=newsPageBean.getTotalPage()%>')">
								</td>
							</tr>
...
<!-- 循环输出数据库中数据 ------------------------------------------------------->
                                        <%
                                            for (Message message:newsPageBean.getArrayList()){
                                        %>
										<tr align="center">
											<td>
												<%=message.getMtitle()%>
											...
											<td width="7%" colspan="2" class="STYLE3">
												<a href="../EmploeNewsDetail?muser=<%=message.getMuser()%>"><img src="../images/detail.gif" /></a>
											</td>
										</tr>
									<%}%>
<!-------------------------------------------------->	
