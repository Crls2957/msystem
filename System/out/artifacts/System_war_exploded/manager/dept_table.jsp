...
<%
	DeptPageBean deptPageBean=(DeptPageBean) request.getSession().getAttribute("dept");
%>
	<body bgcolor="#6CA6CD">
		<center>
						...
						<table width="100%" border="0">
							<tr align="center">
								<td width="58%" height="36" class="STYLE4" align="left">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      		                                                                 第<%=deptPageBean.getPageNow()%>页 ; 共<%=deptPageBean.getLineCount()%>个部门
      		                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      		                        <input type="button"  value="添加部门" onclick="window.location.replace('../ManagerDeptTable_add')" style="color:green;font-size: 15px">
								</td>
								<td width="5%">
									<input type="button" value="首页" onclick="window.location.replace('../ManagerDeptTable?pageNow=1')">
								</td>
								<td width="5%">
									<input type="button" value="上一页" onclick="window.location.replace('../ManagerDeptTable?pageNow=<%=deptPageBean.getPageNow()-1%>')">
								</td>
								<td width="5%">
									<input type="button" value="下一页" onclick="window.location.replace('../ManagerDeptTable?pageNow=<%=deptPageBean.getPageNow()+1%>')">
								</td>
								<td width="5%">
									<input type="button" value="尾页" onclick="window.location.replace('../ManagerDeptTable?pageNow=<%=deptPageBean.getTotalPage()%>')">
								</td>
							</tr>
...
<!-- 循环输出数据库中数据 ------------------------------------------------------->	
										<%
											ArrayList<Dept> deptlist = deptPageBean.getArrayList();
										...
										<tr align="center">
											...
											<td >
												<%
													...
													try{
														emploe = emploeDaoImp.getEmploeById(dept.getEid());
													}catch (Exception e){
														e.printStackTrace();
													}
												 %>
											...
											<td width="7%" class="STYLE3">	
												<a href="../ManagerDeptDelete?did=<%=dept.getDid()%>" onclick="return f_confirm();"><img src="../images/del.gif" width="20"/></a>
											</td>
										</tr>
										<% 	
											}
